...
maximus = Math.max.apply(null, arr);
...
class SpaceShuttle {
	public targetPlanet: string; // declaring a property
	constructor(targetPlanet: string) {
...
class Thermostat {
	...
	to_c = (f: number) => (5 / 9) * (f - 32);
	to_f = (c: number) => (9 * c) / 5 + 32;

	get temperature() {
		return this.to_c(this._temp);
	}
